...
def retained_images():
    return sorted(path for path in FIXTURES.glob('*/*')
    ...
    def test_inventory_pins_every_image_content_dimension_and_label_source(self):
        ...
        for path in retained:
            ...
            with self.subTest(path=relative), Image.open(path) as image:
                ...
                elif '#case=' in label_ref:
                    filename, name = label_ref.split('#case=', 1)
                    self.assertIn(filename, {
                        dataset.get('expectations_file') for dataset in datasets.values()})
                    labels = json.loads((FIXTURES / filename).read_text())
                    self.assertIn(name, {case['path'] for case in labels['cases']})
                else:
