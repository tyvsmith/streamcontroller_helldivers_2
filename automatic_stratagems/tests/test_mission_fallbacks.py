...
class MissionFallbackTests(unittest.TestCase):
    ...
    def test_rover_display_name_is_exact_and_unique(self):
        from automatic_stratagems.scanner import mission_fallbacks
        match_name = mission_fallbacks.match_name
        from automatic_stratagems.scanner.stratagem_detection import catalog
        entries = catalog()
        self.assertEqual(match_name('ROVER', entries)['id'], 'GuardDogRover')
        for text in ('ROVE', 'ROV3R', 'GUARD', 'DOG'):
            with self.subTest(text=text):
                self.assertIsNone(match_name(text, entries)['id'])
        self.assertEqual(match_name('GUARD DOG', entries)['id'], 'GuardDog')

        colliding = {**entries, 'OtherRover': {
            'name': 'Scout Drone', 'sequence': ['UP'],
        }}
        with patch.dict(mission_fallbacks.DISPLAY_NAME_ALIASES,
                        {'OtherRover': ('Rover',)}):
            self.assertIsNone(match_name('ROVER', colliding)['id'])
