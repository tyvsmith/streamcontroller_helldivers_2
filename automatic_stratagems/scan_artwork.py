"""Classify button artwork with Pillow's 0..255 HSV hue scale and draw badges."""
    ...
    with Image.open(path) as source:
        ...
        for (h, s, v), (_, _, _, alpha) in zip(hsv.getdata(), image.getdata()):
            if alpha < 128 or s < 60 or v < 90:
                continue
            # UI filter "blue" includes cyan artwork; recognition names its
            # calibrated OpenCV hue family "cyan".
