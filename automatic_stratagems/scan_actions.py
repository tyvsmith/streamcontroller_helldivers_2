...
def _source_settings(action, settings=None):
    return source_settings(
        action.get_settings() if settings is None else settings,
        action.plugin_base.get_settings())


def _image_page_settings(action):
    return page_source_settings(_source_settings(action))
...
class ScanCoordinator:
    ...
    def open_temporary(self, action, session, *, replace_path=None):
        ...
        path = self.temporary_pages.create(
            ...
            image_settings=_image_page_settings(action))
        ...
        try:
            ...
            if replace_path is not None:
                ...
                try:
                    ...
                    raise
                self.sessions.pop(old_context, None)
                self.state_errors.pop(old_context, None)
        except Exception:
    ...
    def delete_cached_page(self, action, *, preserve_operation=None):
        ...
        if (ScanOperation.action_presentation_is_active(action, source)
                and (preserve_operation is None
                     or self.active_scans.get(source) is not preserve_operation)):
